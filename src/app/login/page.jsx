...
const Login = () => {
    ...
    const focusFirst = useCallback((errors) => {
        ...
        if (firstKey) {
            const element = firstKey === 'email' ? emailRef.current : passwordRef.current;
            if (element) {
                element.focus();
            }
        }
    ...
    const handleMfaChallenge = useCallback(async (mfaResponse) => {
        try {
            setLoading(true);
    ...
    const handleSuccessfulLogin = useCallback(() => {
        ...
        if (successAnnouncementRef.current) {
            successAnnouncementRef.current.textContent = t('login.success.announcement');
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.status === 403 && response.headers.get('X-CSRF-Required') && handleCsrfError) {
                    ...
                    <div className="bg-gray-800/50 border border-gray-700/50 rounded-lg p-8 shadow-2xl">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6" noValidate>
                            ...
                            <button
                                ...
                                disabled={loading || retryAfter > 0}
                                className="w-full bg-gradient-to-r from-orange-600 to-orange-500 hover:from-orange-700 hover:to-orange-600 disabled:opacity-50 disabled:cursor-not-allowed text-white font-medium py-3 px-4 rounded-lg transition-all duration-300 flex items-center justify-center gap-2"
                                aria-describedby={loading ? 'loading-description' : retryAfter > 0 ? 'retry-description' : undefined}
                            >
                                {loading ? (
                                    <>
                                        <div 
                                    ...
                                    <>
                                        <span id="retry-description">{t('login.waitTime', { seconds: retryAfter })}</span>
                                    </>
                                ) : (
                                    t('login.signIn')
                                )}
                        ...
                        <div className="my-6">
                            <div className="relative">
                                ...
                                <div className="relative flex justify-center text-sm">
                                    <span className="px-2 bg-gray-800 text-gray-400">{t('login.orContinueWith')}</span>
                                </div>
                            </div>
                        </div>
