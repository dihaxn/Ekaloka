...
const Analytics = () => {
  ...
  const calculateSummary = data => {
    ...
    const avgConversionRate = (
      data.reduce((sum, d) => sum + parseFloat(d.conversion_rate), 0) /
      data.length
    ).toFixed(2);
                ...
                <div className='bg-gray-800/50 border border-gray-700/50 rounded-lg p-6 hover:shadow-lg hover:shadow-amber-500/10 transition-all duration-300'>
                  <div className='flex items-center justify-between'>
                    <div>
                      ...
                      <p
                        className={`text-xs mt-1 ${summary.growthRate >= 0 ? 'text-green-400' : 'text-red-400'}`}
                      >
                  ...
                  <div className='flex items-center justify-between'>
                    <div>
                      ...
                      <p className='text-green-400 text-xs mt-1'>
                        Avg: {summary.avgOrderValue} per order
                      </p>
                    </div>
                ...
                <div className='bg-gray-800/50 border border-gray-700/50 rounded-lg p-6 hover:shadow-lg hover:shadow-green-500/10 transition-all duration-300'>
                  <div className='flex items-center justify-between'>
                    <div>
                      ...
                      <p className='text-green-400 text-xs mt-1'>
                        {summary.avgConversionRate}% conversion rate
                      </p>
                    </div>
              ...
              <div className='bg-gray-800/50 border border-gray-700/50 rounded-lg p-6'>
                ...
                <div className='h-64 flex items-end justify-between space-x-1'>
                  {analyticsData?.slice(-10).map((data, index) => {
                    const height =
                      (data.revenue /
                        Math.max(...analyticsData.map(d => d.revenue))) *
                      100;
                    return (
                      <div
                        key={index}
                      ...
                      >
                        <div
                          className='bg-gradient-to-t from-amber-600 to-amber-400 rounded-t transition-all duration-300 group-hover:from-amber-500 group-hover:to-amber-300 w-8'
                          style={{ height: `${height}%` }}
                        ></div>
              ...
              <div className='bg-gray-800/50 border border-gray-700/50 rounded-lg p-6'>
                ...
                <div className='h-64 flex items-end justify-between space-x-1'>
                  {analyticsData?.slice(-10).map((data, index) => {
                    const height =
                      (data.orders /
                        Math.max(...analyticsData.map(d => d.orders))) *
                      100;
                    return (
                      <div
                        key={index}
                      ...
                      >
                        <div
                          className='bg-gradient-to-t from-blue-600 to-blue-400 rounded-t transition-all duration-300 group-hover:from-blue-500 group-hover:to-blue-300 w-8'
                          style={{ height: `${height}%` }}
                        ></div>
                        <span className='text-xs text-gray-500 mt-1 rotate-45 transform origin-left'>
                          {new Date(data.date).toLocaleDateString('en-US', {
