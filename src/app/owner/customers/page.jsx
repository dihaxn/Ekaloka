...
const CustomerManagement = () => {
    ...
    const mockCustomers = [
        {
            ...
            loyaltyPoints: 245,
            orders: [
                { id: '#ORD-001', date: '2024-01-15', amount: 249, status: 'Delivered' },
                { id: '#ORD-012', date: '2024-01-10', amount: 399, status: 'Delivered' },
                { id: '#ORD-025', date: '2024-01-05', amount: 159, status: 'Delivered' }
            ]
        },
        {
            id: 'CUST-002',
            ...
            loyaltyPoints: 168,
            orders: [
                { id: '#ORD-002', date: '2024-01-14', amount: 957, status: 'Processing' },
                { id: '#ORD-018', date: '2024-01-08', amount: 299, status: 'Delivered' }
            ]
        },
        {
            id: 'CUST-003',
            ...
            orders: [
                ...
                { id: '#ORD-021', date: '2024-01-07', amount: 329, status: 'Delivered' }
            ]
        },
        {
            id: 'CUST-004',
            ...
            loyaltyPoints: 325,
            orders: [
                { id: '#ORD-004', date: '2024-01-12', amount: 499, status: 'Delivered' },
                { id: '#ORD-015', date: '2024-01-09', amount: 279, status: 'Delivered' }
            ]
        },
        {
            id: 'CUST-005',
            ...
            loyaltyPoints: 35,
            orders: [
                { id: '#ORD-005', date: '2024-01-11', amount: 249, status: 'Cancelled' },
        ...
        },
        {
            ...
            loyaltyPoints: 142,
            orders: [
                { id: '#ORD-010', date: '2024-01-10', amount: 189, status: 'Delivered' },
                { id: '#ORD-022', date: '2023-12-28', amount: 349, status: 'Delivered' }
            ]
        }
    ]
    ...
    const getSegmentColor = (segment) => {
        switch (segment) {
            ...
            case 'Regular': return 'text-blue-400 bg-blue-500/20'
            ...
            <div className="p-6 space-y-6 animate-fade-in-up">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
                    ...
                    <div className="bg-gray-800/50 border border-gray-700/50 rounded-lg p-4">
                        ...
                        <p className="text-2xl font-bold text-amber-400">{currency}{stats.totalRevenue.toLocaleString()}</p>
                    </div>
                    <div className="bg-gray-800/50 border border-gray-700/50 rounded-lg p-4">
                        <p className="text-gray-400 text-sm">Avg AOV</p>
                        <p className="text-2xl font-bold text-purple-400">{currency}{Math.round(stats.averageOrderValue)}</p>
                    </div>
                </div>
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="border-b border-gray-700/50">
                                    <th className="text-left py-3 px-4 text-white font-medium">Customer</th>
                                    <th className="text-left py-3 px-4 text-white font-medium">Segment</th>
                                    <th className="text-left py-3 px-4 text-white font-medium">Orders</th>
                                    <th className="text-left py-3 px-4 text-white font-medium">Total Spent</th>
                                    <th className="text-left py-3 px-4 text-white font-medium">AOV</th>
                                    <th className="text-left py-3 px-4 text-white font-medium">Last Order</th>
                            ...
                            <tbody>
                                {filteredCustomers.map((customer) => (
                                    <tr key={customer.id} className="border-b border-gray-700/30 hover:bg-gray-700/20 transition-colors">
                                        ...
                                        <td className="py-3 px-4 text-gray-300">{customer.totalOrders}</td>
                                        <td className="py-3 px-4">
                                            <span className="text-white font-medium">{currency}{customer.totalSpent.toLocaleString()}</span>
                                        </td>
                                        <td className="py-3 px-4 text-gray-300">{currency}{customer.averageOrderValue}</td>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex justify-between">
                                            ...
                                            <span className="text-white">{new Date(selectedCustomer.joinDate).toLocaleDateString()}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span className="text-gray-400">Favorite Category:</span>
                                            <span className="text-white">{selectedCustomer.favoriteCategory}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                    <div className="bg-gray-900/50 rounded-lg p-4">
                                        <h4 className="text-lg font-bold text-white mb-2">Purchase Summary</h4>
                                        <div className="space-y-2">
                                            <div className="flex justify-between">
                                                <span className="text-gray-400">Total Orders:</span>
                                                <span className="text-white font-medium">{selectedCustomer.totalOrders}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span className="text-gray-400">Total Spent:</span>
                                                <span className="text-white font-medium">{currency}{selectedCustomer.totalSpent.toLocaleString()}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span className="text-gray-400">Average Order:</span>
                                                <span className="text-white font-medium">{currency}{selectedCustomer.averageOrderValue}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span className="text-gray-400">Loyalty Points:</span>
                                                <span className="text-amber-400 font-medium">{selectedCustomer.loyaltyPoints}</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    {selectedCustomer.orders.map((order, index) => (
                                        <div key={index} className="flex justify-between items-center p-3 bg-gray-900/50 rounded-lg">
                                            ...
                                            <div className="text-right">
                                                <p className="text-white font-medium">{currency}{order.amount}</p>
