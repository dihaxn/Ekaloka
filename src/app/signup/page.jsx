...
import { validateEmail, validatePassword, getPasswordErrorMessage } from "../../utils/validators";
import { formatPhoneNumber, validateSriLankanPhone, normalizePhoneForBackend } from "../../utils/phone";
...
const Signup = () => {
    ...
    }, [formData.fullName, formData.email, formData.phoneNumber, formData.subscribeNewsletter, currentStep]);
    ...
    const retryRequest = useCallback((requestFunction) => {
        if (retryCount < 3) {
    ...
    useEffect(() => {
        if (currentStep === 3 && otpSent) {
            ...
            }, 8 * 60 * 1000); // 8 minutes warning (OTP expires in 10 minutes)
    ...
    const validateStep2 = useCallback(() => {
        ...
        if (!formData.phoneNumber.trim()) {
            ...
            errors.phoneNumber = 'Please enter a valid Sri Lankan phone number (07XXXXXXXX or +947XXXXXXXX)';
        }
                        ...
                        {currentStep === 1 && (
                            <form onSubmit={(e) => { e.preventDefault(); handleNext(); }} className="space-y-4" noValidate>
                                ...
                                <div>
                                    ...
                                    <div className="mt-2 text-xs text-gray-500" id="password-hint">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4" noValidate>
                                <div>
                                    ...
                                    <input
                                        ...
                                        disabled={loading}
                                        aria-describedby={validationErrors.otp ? 'otp-error' : undefined}
                                        aria-invalid={!!validationErrors.otp}
                                    />
                                    {validationErrors.otp && (
                                        <p id="otp-error" className="mt-1 text-sm text-red-400" role="alert">
                                            {validationErrors.otp}
                                        </p>
                                    )}
                                </div>
