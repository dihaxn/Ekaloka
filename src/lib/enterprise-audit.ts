...
export class EnterpriseSecurityAudit {
  ...
  static logPrivilegeEscalation(
    ...
    this.logEvent('privilege_escalation', {
      userId,
      attemptedRole,
      currentRole,
      ipAddress,
  ...
  static logBackupEvent(
    event: 'backup_started' | 'backup_completed' | 'backup_failed' | 'restore_started' | 'restore_completed' | 'restore_failed',
    details: Record<string, any>
  ): void {
    this.logEvent('backup_event', {
      event,
      ...details,
      timestamp: new Date().toISOString()
    }, 'medium', { sox: true })
  ...
  static logComplianceEvent(
    ...
    this.logEvent('compliance_event', {
      complianceType,
      event,
      ...details,
      timestamp: new Date().toISOString()
    }, 'medium', { [complianceType]: true })
  }
}
