...
export class EnterpriseCSRFSecurity {
  ...
  static generateToken(): string {
    return crypto
      .randomBytes(ENTERPRISE_SECURITY_CONFIG.CSRF.TOKEN_LENGTH)
      .toString('hex');
  }
   ...
   */
  static generateDoubleSubmitToken(): {
    cookieToken: string;
    headerToken: string;
  } {
...
export class EnterpriseEncryptionSecurity {
  ...
  static encrypt(
    data: string,
    key: string
  ): { encrypted: string; iv: string; tag: string; keyVersion: number } {
    const derivedKey = crypto.scryptSync(
      key,
      'ekaloka-enterprise-salt',
      ENTERPRISE_SECURITY_CONFIG.ENCRYPTION.KEY_LENGTH
    );
    const iv = crypto.randomBytes(
      ENTERPRISE_SECURITY_CONFIG.ENCRYPTION.IV_LENGTH
    );
  ...
  static generateKey(): string {
    return crypto
      .randomBytes(ENTERPRISE_SECURITY_CONFIG.ENCRYPTION.KEY_LENGTH)
      .toString('hex');
  }
}
