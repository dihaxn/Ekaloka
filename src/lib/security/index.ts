...
export class PasswordSecurity {
  ...
  static validatePassword(password: string): {
    ...
    if (
      ...
      !new RegExp(
        `[${SECURITY_CONFIG.PASSWORD_SPECIAL_CHARS.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}]`
      ).test(password)
    ) {
...
export class JWTSecurity {
  ...
  static verifyToken(token: string, secret: 'access' | 'refresh'): any {
    ...
    if (!secretKey) {
      throw new Error(
        `JWT_${secret.toUpperCase()}_SECRET environment variable is not set`
      );
    }
...
export class InputSecurity {
  ...
  ): { isValid: boolean; sanitized: string | null } {
  ...
  static validateFileUpload(file: File): {
    ...
    if (file.size > SECURITY_CONFIG.MAX_FILE_SIZE) {
      errors.push(
        `File size must be less than ${SECURITY_CONFIG.MAX_FILE_SIZE / (1024 * 1024)}MB`
      );
    }
...
export class RateLimitSecurity {
  ...
  static checkRateLimit(
    identifier: string,
    maxAttempts: number = SECURITY_CONFIG.RATE_LIMIT_MAX_REQUESTS
  ): boolean {
    const now = Date.now();
  ...
  static recordAuthFailure(identifier: string): boolean {
    return this.checkRateLimit(
      identifier,
      SECURITY_CONFIG.RATE_LIMIT_MAX_AUTH_ATTEMPTS
    );
  }
