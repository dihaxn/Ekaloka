...
export class EnterpriseRateLimitSecurity {
  ...
  ): { isLimited: boolean; remainingAttempts: number; resetTime: number; blockedUntil?: number } {
    const now = Date.now()
  ...
  static getRateLimitStatus(identifier: string): {
    ...
    if (!record) {
      return {
        count: 0,
        remainingAttempts: ENTERPRISE_SECURITY_CONFIG.RATE_LIMIT.MAX_REQUESTS.GENERAL,
        resetTime: now + ENTERPRISE_SECURITY_CONFIG.RATE_LIMIT.WINDOW_MS,
        isBlocked: false
      }
    }
    ...
    const remainingAttempts = isBlocked ? 0 : Math.max(0, ENTERPRISE_SECURITY_CONFIG.RATE_LIMIT.MAX_REQUESTS.GENERAL - record.count)
  ...
  static removeFromBlacklist(ip: string): void {
    const index = ENTERPRISE_SECURITY_CONFIG.RATE_LIMIT.IP_BLACKLIST.indexOf(ip)
    if (index > -1) {
