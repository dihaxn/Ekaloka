...
export const validators = {
  ...
  maxLength:
    ...
    (value: string | undefined): ValidationError | null => {
      if (!value) return null; // Skip if empty
  ...
  minValue:
    (minValue: number, fieldName: string) =>
  ...
  maxValue:
    (maxValue: number, fieldName: string) =>
    (value: number): ValidationError | null => {
      if (value === null || value === undefined) return null;
  ...
  enum:
    (allowedValues: any[], fieldName: string) =>
...
export type ValidationSchema<T> = {
  [K in keyof T]?: Array<
    (value: T[K], fieldName: string) => ValidationError | null
  >;
};
...
export function validateProductData(data: {
  ...
  const schema: ValidationSchema<typeof data> = {
    ...
    price: [
      validators.required,
      validators.positive('price'),
      validators.maxValue(999999.99, 'price'),
    ],
...
export function validatePaginationParams(data: {
  ...
  if (data.page !== undefined) {
    const page =
      typeof data.page === 'string' ? parseInt(data.page, 10) : data.page;
    if (isNaN(page) || page < 1) {
      errors.push(
        new ValidationError('Page must be a positive number', 'page', data.page)
      );
    }
  }
  ...
  if (data.limit !== undefined) {
    const limit =
      typeof data.limit === 'string' ? parseInt(data.limit, 10) : data.limit;
